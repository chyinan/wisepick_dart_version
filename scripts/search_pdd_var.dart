...
void _usage() {
  print('Usage: dart run scripts/search_pdd_var.dart --keyword KEYWORD [--with-pid true|false] [--with-custom true|false]');
}
...
Future<void> main(List<String> args) async {
  ...
  for (int i = 0; i < args.length; i++) {
    final a = args[i];
    if (a == '--keyword' && i + 1 < args.length) {
      keyword = args[i + 1];
      i++;
    } else if (a == '--with-pid' && i + 1 < args.length) {
      withPid = args[i + 1].toLowerCase() == 'true';
      i++;
    } else if (a == '--with-custom' && i + 1 < args.length) {
      withCustom = args[i + 1].toLowerCase() == 'true';
      i++;
    }
  }
  ...
  if (withPid && Config.pddPid.isNotEmpty) biz['pid'] = Config.pddPid;
  if (withCustom) biz['custom_parameters'] = jsonEncode({'uid': 'chyinan'});

  print('Searching PDD (keyword="$keyword", withPid=$withPid, withCustom=$withCustom) ...');
