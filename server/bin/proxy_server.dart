...
String _normalizeJdImageUrl(dynamic raw) {
  try {
    ...
    if (s.startsWith('jfs/') ||
        s.startsWith('sku/') ||
        s.contains(RegExp(r'jfs/'))) {
      return 'https://img30.360buyimg.com/sku/' +
...
Future<Response> _handleProxy(Request req) async {
  try {
    ...
    return Response(streamedResp.statusCode,
        body: proxiedStream, headers: respHeaders);
  } catch (e) {
    return Response.internalServerError(
        body: jsonEncode({'error': e.toString()}),
        headers: {'content-type': 'application/json'});
...
Future<void> runServer(List<String> args) async {
  ...
  router.post('/admin/login', (Request r) async {
    ...
    if (bodyStr.trim().isNotEmpty) {
      try {
        ...
        if (decoded is Map && decoded['password'] is String) {
          provided = decoded['password'] as String;
        }
      } catch (_) {
        try {
    ...
    if (provided.trim().isEmpty) {
      return Response(400,
          body: jsonEncode({'success': false, 'message': 'password required'}),
          headers: headers);
    }
...
// direct Taobao/JD proxy calls. Keep file tidy without veapi-specific routes.
  ...
  router.get('/__debug/last_return', (Request r) async {
    try {
      ...
      if (asHistory) {
        try {
          return Response.ok(
              jsonEncode({'ok': true, 'history': _lastReturnHistory}),
              headers: {'content-type': 'application/json'});
        } catch (_) {
          // fallback to safe summary
          return Response.ok(
              jsonEncode(
                  {'ok': true, 'history_count': _lastReturnHistory.length}),
              headers: {'content-type': 'application/json'});
        }
      }
      if (_lastReturnDebug == null)
        return Response.ok(jsonEncode({'ok': false, 'msg': 'no debug info'}),
            headers: {'content-type': 'application/json'});
      try {
        return Response.ok(jsonEncode(_lastReturnDebug),
            headers: {'content-type': 'application/json'});
      } catch (_) {
        // fallback to stringified safe summary
        return Response.ok(
            jsonEncode({'ok': true, 'summary': _lastReturnDebug.toString()}),
            headers: {'content-type': 'application/json'});
      }
    } catch (e) {
      return Response.internalServerError(
  ...
  int _safeMatchCount(Map<dynamic, dynamic>? b) {
    ...
    try {
      ...
      if (b.containsKey('tbk_dg_material_optional_upgrade_response')) {
        ...
        if (r is Map &&
            r.containsKey('result_list') &&
            r['result_list'] is Map &&
            r['result_list']['map_data'] is List)
          return (r['result_list']['map_data'] as List).length;
      }
      ...
      if (b.containsKey('tbk_shop_get_response')) {
        ...
        if (r is Map && r.containsKey('results')) {
          final res = r['results'];
          if (res is Map && res['n_tbk_shop'] is List)
            return (res['n_tbk_shop'] as List).length;
        }
      }
  ...
  router.get('/api/products/search', (Request r) async {
    ...
    if (query.isEmpty)
      return Response(400,
          body: jsonEncode({'error': 'query parameter required'}),
          headers: {'content-type': 'application/json'});

    // decide source
    final env = Platform.environment;
    final useTaobao = env['TAOBAO_APP_KEY'] != null &&
        (env['TAOBAO_APP_KEY']?.isNotEmpty ?? false);
    final useJd =
        env['JD_APP_KEY'] != null && (env['JD_APP_KEY']?.isNotEmpty ?? false);
    ...
    try {
      ...
      if (platformParam == 'jd' && useJd) {
        try {
          ...
          if (appKeyEnv == null || appSecretEnv == null)
            throw Exception('JD keys not configured');
          final beijing2 = DateTime.now().toUtc().add(Duration(hours: 8));
          String two2(int n) => n.toString().padLeft(2, '0');
          final ts2 =
              '${beijing2.year}-${two2(beijing2.month)}-${two2(beijing2.day)} ${two2(beijing2.hour)}:${two2(beijing2.minute)}:${two2(beijing2.second)}';
          ...
          sb.write(appSecretEnv);
          final sign =
              md5.convert(utf8.encode(sb.toString())).toString().toUpperCase();
          paramsMap['sign'] = sign;
          http.Response resp;
          String respBody = '';
          int respStatus = 0;
          try {
            resp = await http
                .post(Uri.parse('https://api.jd.com/routerjson'),
                    headers: {
            ...
            if (respStatus != 200) {
              ...
              final dbg = {
                'reqParams': paramsMap,
                'signBase': sb.toString(),
                'respStatus': respStatus,
                'respBody': respBody
              };
              try {
                attemptsLocal
                    .add({'attempt': 'jd_search_http_error', 'debug': dbg});
                final rec = {
                ...
                if (_lastReturnHistory.length > 20)
                  _lastReturnHistory.removeAt(0);
              } catch (_) {}
            ...
            if (!parsed.containsKey('jingdong_search_ware_responce')) {
              ...
              try {
                attemptsLocal.add(
                    {'attempt': 'jd_search_unexpected_body', 'debug': dbg2});
                final rec = {
                  'path': '/api/products/search',
                  'query': query,
                  'err': dbg2,
              ...
              throw Exception(
                  'jingdong.search.ware returned unexpected structure');
            }
          ...
          if (platformParam == 'jd') {
            try {
              attemptsLocal
                  .add({'attempt': 'jd_search_failed', 'debug': e.toString()});
            } catch (_) {}
            final outEmpty = {
              'products': <dynamic>[],
              'source': 'jd',
            ...
            return Response.ok(jsonEncode(outEmpty),
                headers: {'content-type': 'application/json'});
          }
          ...
          if (useTaobao) {
            ...
            return Response.internalServerError(
                body: jsonEncode({'error': 'no data source configured'}),
                headers: {'content-type': 'application/json'});
          }
        }
        ...
        try {
          ...
          if (pddClientId == null ||
              pddClientSecret == null ||
              pddClientId.isEmpty) {
            return Response.internalServerError(
                body: jsonEncode({
                  'error': 'PDD_CLIENT_ID/PDD_CLIENT_SECRET not configured'
                }),
                headers: {'content-type': 'application/json'});
          }
          ...
          if (params.containsKey('with_coupon'))
            biz['with_coupon'] = params['with_coupon'];
          ...
          final respP = await http
              .post(Uri.parse('https://gw-api.pinduoduo.com/api/router'),
                  headers: {
                    'Content-Type': 'application/x-www-form-urlencoded'
                  },
                  body: form)
          ...
          if (respP.statusCode != 200) {
            ...
            attemptsLocal
                .add({'attempt': 'pdd_search_http_error', 'debug': dbg});
            return Response.internalServerError(
                body: jsonEncode({'error': 'pdd api failed', 'detail': dbg}),
                headers: {'content-type': 'application/json'});
          }

          final Map<String, dynamic> pbody =
              jsonDecode(respP.body) as Map<String, dynamic>;
          final goodsResp = (pbody['goods_search_response'] is Map)
              ? pbody['goods_search_response'] as Map<String, dynamic>
              : null;
            ...
            for (final it in gl) {
              if (it is Map) {
                ...
                try {
                  ...
                  final image =
                      (m['goods_image_url'] ?? m['goods_thumbnail_url'] ?? '')
                          .toString();
                  final minGroup = ((m['min_group_price'] is num)
                          ? (m['min_group_price'] as num).toDouble()
                          : (double.tryParse(
                                  (m['min_group_price'] ?? '0').toString()) ??
                              0.0)) /
                      100.0;
                  final minNorm = ((m['min_normal_price'] is num)
                          ? (m['min_normal_price'] as num).toDouble()
                          : (double.tryParse(
                                  (m['min_normal_price'] ?? '0').toString()) ??
                              0.0)) /
                      100.0;
                  final coupon = ((m['coupon_discount'] is num)
                          ? (m['coupon_discount'] as num).toDouble()
                          : (double.tryParse(
                                  (m['coupon_discount'] ?? '0').toString()) ??
                              0.0)) /
                      100.0;
                  int sales = 0;
                  try {
                    ...
                    if (st.contains('万')) {
                      ...
                      if (wm != null)
                        sales = (double.parse(wm.group(1)!) * 10000).toInt();
                    } else {
                  ...
                  final shopTitle = (m['brand_name'] ??
                      ...
                      '';
                  final desc =
                      (m['goods_desc'] ?? m['desc_txt'] ?? '').toString();
                  ...
                  pProducts.add({
                    ...
                    'rating': 0.0,
                    'link': '',
                    'commission': ((m['promotion_rate'] is num)
                            ? (m['promotion_rate'] as num).toDouble()
                            : (double.tryParse(
                                    (m['promotion_rate'] ?? '0').toString()) ??
                                0.0)) /
                        1000.0 *
                        minGroup,
                    'description': desc,
            ...
            if (!needFallbackToSearchWare) {
              ...
              try {
                ...
                if (resp2.statusCode != 200)
                  throw Exception(
                      'jingdong.search.ware failed ${resp2.statusCode}');
      ...
      if (body is Map &&
        ...
        try {
          ...
          if (respMap is Map &&
              respMap['result_list'] is Map &&
              respMap['result_list']['map_data'] is List) {
          ...
          if (paras != null && paras is List) {
            ...
            for (final it in _paras) {
              ...
              try {
                ...
                if (it.containsKey('price'))
                  price =
                      (num.tryParse(it['price'].toString()) ?? 0).toDouble();
              } catch (_) {}

              final link =
                  (it['itemUrl'] ?? it['materialUrl'] ?? '').toString();
              ...
              try {
                if ((finalId == null || finalId.isEmpty) &&
                  ...
                  if (sw is Map && sw['wareid'] != null)
                    finalId = sw['wareid'].toString();
                }
              } catch (_) {}
      ...
      if (items != null) {
        for (final it in items) {
          if (it is Map<String, dynamic>) {
            ...
            if (it.containsKey('item_basic_info') &&
                it['item_basic_info'] is Map) {
              final basic = it['item_basic_info'] as Map<String, dynamic>;
              final id =
                  (it['item_id'] ?? it['item_id'] ?? it['id'] ?? '').toString();
              final title =
                  (basic['title'] ?? basic['short_title'] ?? '').toString();
              String imageUrl = (basic['pict_url'] ??
                      basic['pic_url'] ??
                      basic['white_image'] ??
                      '')
                  .toString();
              if (imageUrl.isEmpty &&
                  basic['small_images'] is Map &&
                  basic['small_images']['string'] is List) {
                final list = basic['small_images']['string'] as List;
                if (list.isNotEmpty) imageUrl = list[0].toString();
              }
              double price = 0.0;
              try {
                final cand = basic['zk_final_price'] ??
                    basic['reserve_price'] ??
                    (it['price_promotion_info'] is Map
                        ? (it['price_promotion_info']
                                ['final_promotion_price'] ??
                            it['price_promotion_info']['zk_final_price'] ??
                            it['price_promotion_info']['reserve_price'])
                        : null);
              ...
              double coupon = 0.0;
              try {
                final cnd = it['coupon_amount'] ??
                    it['coupon'] ??
                    (it['price_promotion_info'] is Map
                        ? it['price_promotion_info']['coupon_amount'] ??
                            it['price_promotion_info']['coupon']
                        : null);
              ...
              try {
                ...
                final fp = (it['price_promotion_info'] is Map
                    ? (it['price_promotion_info']['final_promotion_price'] ??
                        it['price_promotion_info']['final_promotion_price'])
                    : null);
                if (fp != null && fp.toString().isNotEmpty) {
                  finalPrice = (num.tryParse(fp.toString()) ?? (price - coupon))
                      .toDouble();
                } else {
              ...
              int sales = (num.tryParse((basic['volume'] ??
                      ...
                      0)
                  .toInt();
              double commission = (num.tryParse(
                          (it['commission'] ?? it['commission_rate'] ?? '0')
                              .toString()) ??
                      0)
                  .toDouble();
              // Prefer coupon_share_url in publish_info, then publish_info.click_url, then other fields
              final link = (it['publish_info'] is Map &&
                      it['publish_info']['coupon_share_url'] != null)
                  ? it['publish_info']['coupon_share_url'].toString()
                  : (it['publish_info'] is Map &&
                          it['publish_info']['click_url'] != null)
                ...
                for (final k in keys) {
                  ...
                  try {
                    if (products.isEmpty &&
                      ...
                      List<dynamic>? paras2;
                      if (rawJd2.containsKey('Paragraph') &&
                          rawJd2['Paragraph'] is List)
                        paras2 = rawJd2['Paragraph'] as List<dynamic>;
                      else if (rawJd2.containsKey('Head') &&
                          rawJd2['Head'] is Map &&
                          rawJd2['Head']['Paragraph'] is List)
                        paras2 = rawJd2['Head']['Paragraph'] as List<dynamic>;
                      if (paras2 != null) {
                        for (final it in paras2) {
                          try {
                            if (it is Map) {
                              final id = (it['wareid'] ?? it['wareId'] ?? '')
                                  .toString();
                              String title = '';
                              try {
                                if (it['Content'] is Map)
                                  title = (it['Content']['warename'] ??
                                          it['Content']['wareName'] ??
                                          '')
                                      .toString();
                                if (title.isNotEmpty)
                              ...
                              try {
                                if (it['Content'] is Map &&
                                    it['Content']['imageurl'] != null)
                                  imageUrl = _normalizeJdImageUrl(
                                      it['Content']['imageurl']);
                                else if (it['SlaveParagraph'] is List &&
                                    it['SlaveParagraph'].isNotEmpty) {
                                  final sp = it['SlaveParagraph'][0];
                                  if (sp is Map &&
                                      sp['SlaveContent'] is Map &&
              ...
              pmap = {
                ...
                // prefer short_title over sub_title for display
                'description':
                    (it['short_title'] ?? it['sub_title'] ?? it['title'] ?? '')
                        .toString(),
              };
            } else {
              // taobao mapping best-effort
              String id = (it['num_iid'] ??
                      it['id'] ??
                      it['item_id'] ??
                      it['goods_id'] ??
                      '')
                  .toString();
              String title =
                  (it['title'] ?? it['item_title'] ?? it['name'] ?? '')
                      .toString();
              String imageUrl = (it['pic_url'] ??
                      it['pict_url'] ??
                      it['small_images'] ??
                      it['image_url'] ??
                      '')
              ...
              if (imageUrl.isEmpty &&
                  it['small_images'] is Map &&
                  it['small_images']['string'] is List) {
                final list = it['small_images']['string'] as List;
                if (list.isNotEmpty) imageUrl = list[0].toString();
              }
              double price = 0.0;
              try {
                price = (num.tryParse((it['zk_final_price'] ??
                                it['reserve_price'] ??
                                it['price'] ??
                                '0')
                            .toString()) ??
              ...
              double coupon = (num.tryParse(
                          (it['coupon_amount'] ?? it['coupon'] ?? '0')
                              .toString()) ??
                      0)
                  .toDouble();
              double finalPrice = (num.tryParse(
                          (it['final_price'] ?? it['after_coupon_price'] ?? '')
                              .toString()) ??
                      (price - coupon))
                  .toDouble();
              ...
              double commission = (num.tryParse(
                  ...
                  .toDouble();
              String link =
                  (it['click_url'] ?? it['url'] ?? it['coupon_click_url'] ?? '')
                      .toString();
              ...
              for (final it in jdItems) {
                if (it is Map<String, dynamic>) {
                  try {
                    ...
                    String imageUrl = '';
                    try {
                      if (it['imageInfo'] is Map &&
                          it['imageInfo']['imageList'] is List &&
                          it['imageInfo']['imageList'].isNotEmpty)
                        imageUrl =
                            it['imageInfo']['imageList'][0]['url'].toString();
                    } catch (_) {}
                    if (imageUrl.isEmpty)
                      imageUrl =
                          (it['imageUrl'] ?? it['imageUrl'] ?? '').toString();
                    double price = 0.0;
                    try {
                      ...
                      if (it['priceInfo'] is Map) {
                        ...
                        candidates.add(pi['price'] ??
                            pi['promotionPrice'] ??
                            pi['unitPrice'] ??
                            pi['lowestPrice'] ??
                            pi['priceInfo']);
                      }
                      candidates.add(it['price'] ??
                          it['finalPrice'] ??
                          it['priceInfo'] ??
                          it['jdPrice'] ??
                          it['lowestCouponPrice'] ??
                          it['coupon_price'] ??
                          it['zk_final_price'] ??
                          it['priceInfo']);
                    ...
                    try {
                      if (it['couponInfo'] is Map) {
                        ...
                        if (ci['couponList'] is Map &&
                            ci['couponList']['coupon'] is Map) {
                          coupon = (num.tryParse(ci['couponList']['coupon']
                                              ['discount']
                                          ?.toString() ??
                                      '') ??
                                  0)
                              .toDouble();
                        } else if (ci['couponList'] is List &&
                            ci['couponList'].isNotEmpty) {
                          coupon = (num.tryParse(ci['couponList'][0]['discount']
                                          ?.toString() ??
                                      '') ??
                                  0)
                              .toDouble();
                    ...
                    String link = (it['materialUrl'] ??
                                it['couponInfo'] is Map &&
                                    it['couponInfo']['couponList'] is Map
                            ? (it['couponInfo']['couponList']['coupon'] is Map
                                ? it['couponInfo']['couponList']['coupon']
                                    ['link']
                                : null)
                            : null) as String? ??
                        '';
                    ...
                    try {
                      commission = (it['commissionInfo'] != null &&
                              it['commissionInfo']['commission'] != null)
                          ? (num.tryParse(it['commissionInfo']['commission']
                                      .toString()) ??
                                  0)
                              .toDouble()
                          : 0.0;
                    } catch (_) {}
                    int sales = (num.tryParse((it['inOrderCount30Days'] ??
                                    it['comments'] ??
                                    '0')
                                .toString()) ??
                            0)
                    ...
                    if ((usedPrice == 0.0 || usedPrice == null) &&
                      ...
                      try {
                        ...
                        if (entry != null &&
                            (entry['expiry'] as int?) != null &&
                            (entry['expiry'] as int) > nowMs &&
                            entry['price'] != null) {
                      ...
                      if ((usedPrice == 0.0 || usedPrice == null)) {
                        try {
                          ...
                          final client = http.Client();
                          final resp =
                              await client.get(Uri.parse(itemUrl), headers: {
                            'User-Agent':
      ...
      if (items == null) {
        ...
        if (mlist != null) {
          for (final it in mlist) {
            if (it is Map<String, dynamic>) {
              final id =
                  (it['num_iid'] ?? it['item_id'] ?? it['id'] ?? '').toString();
              final title =
                  (it['title'] ?? it['item_title'] ?? it['name'] ?? '')
                      .toString();
              final imageUrl = (it['pict_url'] ??
                      it['pic_url'] ??
                      it['small_images'] ??
                      it['image_url'] ??
                      '')
                  .toString();
              final price = (num.tryParse((it['zk_final_price'] ??
                              it['coupon_price'] ??
                              it['price'] ??
                              '0')
                          .toString()) ??
                      0)
                  .toDouble();
              final sales = (num.tryParse((it['volume'] ??
                              it['sales'] ??
                              it['tk_total_sales'] ??
                              '0')
                          .toString()) ??
              ...
              final link = (it['coupon_share_url'] ??
                      it['click_url'] ??
                      it['coupon_click_url'] ??
                      it['purl'] ??
                      it['clickUrl'] ??
              ...
              products.add({
                ...
                'short_title': (it['short_title'] ?? '').toString(),
                'description':
                    (it['sub_title'] ?? it['short_title'] ?? it['title'] ?? '')
                        .toString(),
              });
            }
          }
      ...
      try {
        ...
        final pddClientSecret = env['PDD_CLIENT_SECRET'];
        final pddPid = env['PDD_PID'] ?? '';
        final wantPdd = pddClientId != null &&
            pddClientSecret != null &&
            pddClientId.isNotEmpty;
        if (wantPdd &&
            (platformParam == 'all' ||
                platformParam.isEmpty ||
                platformParam == 'pdd')) {
          ...
          try {
            ...
            if (respP.statusCode == 200) {
              ...
              try {
                ...
                await f
                    .writeAsString(JsonEncoder.withIndent('  ').convert(pbody));
              } catch (_) {}
              final goodsResp = (pbody['goods_search_response'] is Map)
                  ? pbody['goods_search_response'] as Map<String, dynamic>
                  : null;
              if (goodsResp != null && goodsResp['goods_list'] is List) {
                final List<dynamic> gl =
                    goodsResp['goods_list'] as List<dynamic>;
                for (final it in gl) {
                  try {
                    if (it is Map) {
                      final m = Map<String, dynamic>.from(it as Map);
                      ...
                      final image = (m['goods_image_url'] ??
                              m['goods_thumbnail_url'] ??
                              '')
                          .toString();
                      final minGroup = ((m['min_group_price'] is num)
                              ? (m['min_group_price'] as num).toDouble()
                              : (double.tryParse((m['min_group_price'] ?? '0')
                                      .toString()) ??
                                  0.0)) /
                          100.0;
                      final minNorm = ((m['min_normal_price'] is num)
                              ? (m['min_normal_price'] as num).toDouble()
                              : (double.tryParse((m['min_normal_price'] ?? '0')
                                      .toString()) ??
                                  0.0)) /
                          100.0;
                      final coupon = ((m['coupon_discount'] is num)
                              ? (m['coupon_discount'] as num).toDouble()
                              : (double.tryParse((m['coupon_discount'] ?? '0')
                                      .toString()) ??
                                  0.0)) /
                          100.0;
                      int sales = 0;
                      try {
                        ...
                        if (st.contains('万')) {
                          final wm =
                              RegExp(r"([0-9]+\.?[0-9]*)万").firstMatch(st);
                          if (wm != null)
                            sales =
                                (double.parse(wm.group(1)!) * 10000).toInt();
                        } else {
                          final n = RegExp(r"(\d+)").firstMatch(st)?.group(1);
                          if (n != null) sales = int.tryParse(n) ?? 0;
                        }
                      ...
                      final shopTitle =
                          (m['mall_name'] ?? m['opt_name'] ?? '').toString();
                      final desc =
                          (m['goods_desc'] ?? m['desc_txt'] ?? '').toString();
                      ...
                      products.add({
                        ...
                        'commission': ((m['promotion_rate'] is num)
                                ? (m['promotion_rate'] as num).toDouble()
                                : (double.tryParse((m['promotion_rate'] ?? '0')
                                        .toString()) ??
                                    0.0)) /
                            1000.0 *
                            minGroup,
      ...
      try {
        if (body is Map && body.containsKey('jingdong_search_ware_responce')) {
          ...
          if (paras != null && paras.isNotEmpty) {
            final existingIds =
                products.map((p) => (p['id']?.toString() ?? '')).toSet();
                ...
                if (it is Map) {
                  ...
                  try {
                    if (it['Content'] is Map &&
                        it['Content']['imageurl'] != null)
                      imageUrl =
                          _normalizeJdImageUrl(it['Content']['imageurl']);
                    else if (it['SlaveWare'] is List &&
                        it['SlaveWare'].isNotEmpty) {
                      final sw = it['SlaveWare'][0];
                      if (sw is Map &&
                          sw['Content'] is Map &&
                          sw['Content']['imageurl'] != null)
                        imageUrl =
                            _normalizeJdImageUrl(sw['Content']['imageurl']);
                    }
                  } catch (_) {}
                  ...
                  try {
                    if (it['price'] != null)
                      price = (num.tryParse(it['price'].toString()) ?? 0)
                          .toDouble();
                  } catch (_) {}

                  final link =
                      (it['materialUrl'] ?? it['itemUrl'] ?? '').toString();
                  final sales = (num.tryParse(
                              (it['good'] ?? it['sales'] ?? '0').toString()) ??
                          0)
                      .toInt();
      ...
      final sortName = (params['sortName'] ?? params['sort_name'] ?? '')
          .toString()
          .toLowerCase();
      ...
      final page = int.tryParse((params['page'] ??
              ...
              .toString()) ??
          1;
      final pageSize = int.tryParse((params['page_size'] ??
                  params['pageSize'] ??
                  params['pageSize'] ??
                  '20')
              .toString()) ??
        ...
        for (int i = 0; i < finalProducts.length; i++) {
          ...
          try {
            if ((p['platform']?.toString() ?? '') == 'jd') {
              final num? curPriceNum = (p['price'] is num)
                  ? (p['price'] as num)
              ...
              if (needFetch) {
                _priceFutures.add(Future(() async {
                  ...
                  try {
                    ...
                    if (cache != null &&
                        (cache['expiry'] as int?) != null &&
                        (cache['expiry'] as int) > nowMs &&
                        cache['price'] != null) {
                      final double cp = (cache['price'] as num).toDouble();
                      p['price'] = cp;
                      p['original_price'] = cp;
                      p['final_price'] =
                          (cp - ((p['coupon'] as num?) ?? 0)).toDouble();
                      return;
                    }

                    final itemUrl = Uri.parse(
                        'https://item.jd.com/${Uri.encodeComponent(id)}.html');
                    final client = http.Client();
                    ...
                    if (resp.statusCode == 200) {
                      ...
                      if (found > 0) {
                        ...
                        p['final_price'] =
                            (found - ((p['coupon'] as num?) ?? 0)).toDouble();
  ...
  router.get('/taobao/tbk_search', (Request r) async {
    ...
    if (appKey == null ||
      ...
      return Response.internalServerError(
          body: jsonEncode(
              {'error': 'TAOBAO_APP_KEY/TAOBAO_APP_SECRET not configured'}),
          headers: {'content-type': 'application/json'});
    }
    ...
    final requestedMethod =
        params['method'] ?? 'taobao.tbk.dg.material.optional.upgrade';
    ...
    if (params.containsKey('page_no'))
      apiParams['page_no'] = params['page_no']!;
    if (params.containsKey('page_size'))
      apiParams['page_size'] = params['page_size']!;
    // 默认使用用户提供的推广位（adzone_id），若未传则使用项目默认推广位
    if (params.containsKey('adzone_id'))
      apiParams['adzone_id'] = params['adzone_id']!;
    else
      apiParams['adzone_id'] = params['adzone_id'] ?? '116145250221';
    if (params.containsKey('site_id'))
      apiParams['site_id'] = params['site_id']!;
    if (params.containsKey('material_id'))
      apiParams['material_id'] = params['material_id']!;
    if (params.containsKey('sort')) apiParams['sort'] = params['sort']!;
    if (params.containsKey('has_coupon'))
    ...
    apiParams['timestamp'] =
        beijing.toIso8601String().split('.').first.replaceFirst('T', ' ');
    ...
    final signDigest =
        md5.convert(utf8.encode(signBase)).toString().toUpperCase();
    ...
    try {
      ...
      if (params['debug_sign'] == '1') {
        ...
        return Response.ok(jsonEncode(debugBody),
            headers: {'content-type': 'application/json'});
      }
      ...
      if (resp.statusCode != 200)
        return Response.internalServerError(
            body: jsonEncode(
                {'error': 'taobao api failed', 'status': resp.statusCode}),
            headers: {'content-type': 'application/json'});
  ...
  router.get('/proxy/test/jd_search', (Request r) async {
    ...
    if (keyword.isEmpty)
      return Response(400,
          body: jsonEncode({'error': 'keyword required'}),
          headers: {'content-type': 'application/json'});

    final beijing = DateTime.now().toUtc().add(Duration(hours: 8));
    String two(int n) => n.toString().padLeft(2, '0');
    final ts =
        '${beijing.year}-${two(beijing.month)}-${two(beijing.day)} ${two(beijing.hour)}:${two(beijing.minute)}:${two(beijing.second)}';
    ...
    try {
      final resp = await http
          .post(Uri.parse('https://api.jd.com/routerjson'),
              ...
              body: paramsMap)
          .timeout(Duration(seconds: 8));
      return Response(resp.statusCode, body: resp.body, headers: {
  ...
  router.get('/jd/union/goods/query', (Request r) async {
    ...
    if (keyword.isEmpty)
      return Response(400,
          body: jsonEncode({'error': 'keyword parameter required'}),
          headers: {'content-type': 'application/json'});
    ...
    final goodsReq = {
      'keyword': keyword,
      'pageIndex': int.tryParse(pageIndex) ?? 1,
      'pageSize': int.tryParse(pageSize) ?? 20
    };
    ...
    if (params['debug_sign'] == '1') {
      final debugBody = {
        ...
        'signBase': signBase,
        'sign': apiParams['sign'],
        'called_method': apiParams['method']
      };
      return Response.ok(jsonEncode(debugBody),
          headers: {'content-type': 'application/json'});
    }
    ...
    try {
      ...
      if (resp.statusCode != 200)
        return Response.internalServerError(
            body: jsonEncode(
                {'error': 'jd api failed', 'status': resp.statusCode}),
            headers: {'content-type': 'application/json'});
  ...
  router.post('/sign/jd', (Request r) async {
    ...
    final appKey = (env['JD_APP_KEY'] ?? 'd26362a7d1037522f81c57afc70a78e7')
        .toString()
        .trim();
    final appSecret =
        (env['JD_APP_SECRET'] ?? '25638a02874e47858929243487818e72')
            .toString()
            .trim();
    ...
    final skuId = payload['skuId']?.toString() ??
        payload['sku_id']?.toString() ??
        payload['id']?.toString() ??
        '';
    final couponUrl = payload['couponUrl']?.toString() ??
        payload['coupon_url']?.toString() ??
        '';
    // Allow payload to override env for optional promotion fields
    final payloadSiteId =
        payload['siteId']?.toString() ?? payload['site_id']?.toString() ?? '';
    final payloadMaterialId = payload['materialId']?.toString() ??
        payload['material_id']?.toString() ??
        '';
    final payloadPositionId = payload['positionId']?.toString() ??
        payload['position_id']?.toString() ??
        '';
    final payloadSubUnion = payload['subUnionId']?.toString() ??
        payload['sub_union_id']?.toString() ??
        payload['subUnion']?.toString() ??
        '';
    final payloadUnionId =
        payload['unionId']?.toString() ?? payload['union_id']?.toString() ?? '';
    final payloadSceneId =
        payload['sceneId']?.toString() ?? payload['scene_id']?.toString() ?? '';
    ...
    final siteIdEnv =
        env['JD_SITE_ID'] ?? env['JD_SITEID'] ?? env['JD_SITE'] ?? '4102034645';
    final positionIdEnv =
        env['JD_POSITION_ID'] ?? env['JD_POSITIONID'] ?? env['JD_PID'];
    final subUnionIdEnv =
        env['JD_SUB_UNION_ID'] ?? env['JD_SUBUNIONID'] ?? env['JD_SUB_UNION'];
    ...
    if (payloadSubUnion.isNotEmpty)
      biz['subUnionId'] = payloadSubUnion;
    else if (subUnionIdEnv != null && subUnionIdEnv.isNotEmpty)
      biz['subUnionId'] = subUnionIdEnv;
    ...
    if (!biz.containsKey('materialId') ||
        (biz['materialId'] is String &&
            (biz['materialId'] as String).isEmpty)) {
      if (skuId.isNotEmpty) {
        try {
    ...
    if (!debugSign) {
      try {
        if (payload.containsKey('debug_sign') &&
            (payload['debug_sign'] == true || payload['debug_sign'] == '1'))
          debugSign = true;
      } catch (_) {}
    }
    if (debugSign) {
    ...
    try {
      ...
      try {
        ...
        if (parsedResp
            .containsKey('jd_union_open_promotion_common_get_responce')) {
          final inner =
              parsedResp['jd_union_open_promotion_common_get_responce'];
      ...
      try {
        final shouldRetry = parsedGetResult != null &&
            (parsedGetResult['code']?.toString() == '2001701' ||
                (parsedGetResult['message'] is String &&
                    (parsedGetResult['message'] as String)
                        .contains('不支持siteId')));
        if (shouldRetry) {
          ...
          final signRetry =
              md5.convert(utf8.encode(signBaseRetry)).toString().toUpperCase();
          apiParams['sign'] = signRetry;

          try {
            final respRetry = await http
                .post(uri,
                    headers: {
                      'Content-Type':
                          'application/x-www-form-urlencoded;charset=utf-8'
            ...
            try {
              final parsedResp2 =
                  jsonDecode(respBodyFinal) as Map<String, dynamic>;
              if (parsedResp2
                  .containsKey('jd_union_open_promotion_common_get_responce')) {
                final inner2 =
                    parsedResp2['jd_union_open_promotion_common_get_responce'];
                if (inner2 is Map && inner2['getResult'] != null) {
                  final raw2 = inner2['getResult'];
                  final rawStr2 = raw2 is String ? raw2 : jsonEncode(raw2);
                  try {
                    parsedGetResult =
                        jsonDecode(rawStr2) as Map<String, dynamic>;
                  } catch (_) {}
                }
              }
            } catch (_) {}
      ...
      try {
        ...
        return Response.internalServerError(
            body: jsonEncode(
                {'error': 'jd response not json', 'debug': debugRecord}),
            headers: {'content-type': 'application/json'});
      }
        ...
        void walk(Map m) {
          for (final e in m.entries) {
            ...
            if (v is String) {
              ...
              if (clickUrl == null &&
                  (key.contains('clickurl') ||
                      key.contains('click_url') ||
                      key.contains('click'))) clickUrl = v;
      ...
      if ((clickUrl == null || clickUrl!.isEmpty) &&
          (tpwd == null || tpwd!.isEmpty)) {
        // If JD returned a parseable business result, surface that to the client
        final innerCode =
            parsedGetResult != null && parsedGetResult.containsKey('code')
                ? parsedGetResult['code']
                : null;
        final innerMsg =
            parsedGetResult != null && parsedGetResult.containsKey('message')
                ? parsedGetResult['message']
                : null;
        if (innerCode != null && innerCode.toString() != '200') {
          return Response(400,
              body: jsonEncode({
                'error': 'jd_business_error',
                'code': innerCode,
                'message': innerMsg,
                'debug': debugRecord
              }),
        ...
        return Response.internalServerError(
            body: jsonEncode(
                {'error': 'no promotion link returned', 'debug': debugRecord}),
            headers: {'content-type': 'application/json'});
      }
      ...
      return Response.internalServerError(
          body: jsonEncode({'error': e.toString(), 'debug': rec}),
          headers: {'content-type': 'application/json'});
    }
  });
  ...
  router.post('/pdd/authority/query', (Request r) async {
    try {
      ...
      final clientSecret =
          env['PDD_CLIENT_SECRET'] ?? env['PDD_CLIENTSECRET'] ?? '';
      ...
      final pidRaw =
          (payload['pid'] ?? payload['p_id'] ?? '')?.toString() ?? '';
      String pid = pidRaw;
      if (pid.isEmpty && env['PDD_PID'] != null) pid = env['PDD_PID']!;
      // ensure pid has underscore after 8 chars
      if (pid.isNotEmpty && !pid.contains('_') && pid.length > 8)
        pid = pid.substring(0, 8) + '_' + pid.substring(8);

      final customParameters = payload['custom_parameters'] ??
          payload['customParameters'] ??
          payload['customParametersJson'] ??
          '';
      final biz = <String, dynamic>{};
      if (pid.isNotEmpty) biz['pid'] = pid;
      if (customParameters != null && customParameters.toString().isNotEmpty)
        biz['custom_parameters'] = customParameters is String
            ? customParameters
      ...
      return Response(upstream.statusCode, body: upstream.body, headers: {
  ...
  router.post('/sign/pdd', (Request r) async {
    try {
      ...
      try {
        final rawList = payload['goods_sign_list'] ??
            payload['goodsSignList'] ??
            payload['goods_sign_list[]'] ??
            payload['goods_sign'] ??
            payload['goodsSign'] ??
            '';
        if (rawList is List)
          goodsSignList = rawList.map((e) => e.toString()).toList();
        else if (rawList is String && rawList.isNotEmpty) {
          // try parse JSON array string
          try {
            final dec = jsonDecode(rawList);
            if (dec is List)
              goodsSignList = dec.map((e) => e.toString()).toList();
            else
      ...
      if (goodsSignList.isEmpty)
        return Response(400,
            body:
                jsonEncode({'error': 'goods_sign or goods_sign_list required'}),
            headers: {'content-type': 'application/json'});
      ...
      final biz = <String, dynamic>{
        ...
        'p_id': pid,
        'custom_parameters': customParameters is String
            ? customParameters
            : jsonEncode(customParameters),
      };
      ...
      try {
        if (resp.statusCode != 200) {
          ...
          return Response.internalServerError(
              body: jsonEncode(
                  {'error': 'pdd promotion api failed', 'detail': resp.body}),
              headers: {'content-type': 'application/json'});
        }
        ...
        return Response.internalServerError(
            body: jsonEncode({
              'error': 'pdd promotion parse failed',
              'detail': e.toString()
            }),
            headers: {'content-type': 'application/json'});
      }
      ...
      final genResp = (pbody['goods_promotion_url_generate_response'] is Map)
          ? pbody['goods_promotion_url_generate_response']
              as Map<String, dynamic>
          : null;
      String clickURL = '';
      try {
        if (genResp != null &&
            genResp['goods_promotion_url_list'] is List &&
            (genResp['goods_promotion_url_list'] as List).isNotEmpty) {
          final entry = (genResp['goods_promotion_url_list'] as List).first
              as Map<String, dynamic>;
          // Prefer short_url (short link) -> mobile_short_url -> mobile_url -> url
          clickURL = (entry['short_url'] ??
                  entry['mobile_short_url'] ??
                  entry['mobile_url'] ??
                  entry['url'] ??
                  '')
              .toString();
      ...
      return Response.ok(jsonEncode({'clickURL': clickURL, 'raw': pbody}),
          headers: {'content-type': 'application/json'});
    } catch (e) {
      return Response.internalServerError(
          body: jsonEncode({'error': e.toString()}),
  ...
  router.get('/api/get-jd-promotion', (Request r) async {
    ...
    final pythonExecutable =
        Platform.isWindows ? r'.venv\Scripts\python.exe' : '.venv/bin/python';
    final scriptPath =
        Platform.isWindows ? r'bin\jd_scraper.py' : 'bin/jd_scraper.py';
    ...
    try {
      ...
      if (exitCode == 0) {
        ...
        if (jsonResult['status'] == 'success') {
          ...
          final linkMatch =
              RegExp(r'https?://u\.jd\.com/[A-Za-z0-9]+').firstMatch(content);
          final priceMatch =
              RegExp(r'京东价：¥([0-9]+(?:\.[0-9]+)?)').firstMatch(content);
          ...
          return Response.internalServerError(
              body: jsonEncode({
                'status': 'error',
                'message': 'Python script failed: ${jsonResult['message']}'
              }),
              headers: {'Content-Type': 'application/json'});
        }
      } else {
        return Response.internalServerError(
            body: jsonEncode({
              'status': 'error',
              'message':
                  'Python script execution failed (Exit Code: $exitCode)',
      ...
      return Response.internalServerError(
          body: jsonEncode({
            'status': 'error',
            'message': 'Failed to launch Python script: $e'
          }),
          headers: {'Content-Type': 'application/json'});
    }
  });
...
void main(List<String> args) async {
  ...
  Future<void> writeDotEnv(File f, Map<String, String> map) async {
    try {
      ...
      buf.writeln(
          '# WARNING: Do NOT commit this file to version control. It may contain secrets.');
  ...
  final Map<String, String> mergedEnv =
